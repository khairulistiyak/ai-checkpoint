...
export default function GitVisualizer({ projectId, onRefresh }) {
  ...
  if (checkpoints.length === 0) {
    return (
      <div className="mt-16">
        <h2 className="text-xl font-bold text-white mb-6 pl-2 flex items-center gap-2">
          <GitCommit className="w-5 h-5 text-violet-400" /> Git Checkpoints
        </h2>
        <div className="glass-card p-8 text-center text-slate-400 border-dashed border-slate-700">
          No checkpoints found. Ensure you are using `aicp/` tags.
        </div>
      </div>
    );
  }
      ...
      <div className="glass-card p-6 relative">
        ...
        <div className="space-y-6">
          {checkpoints.map((cp, idx) => (
            ...
            >
              <div className="w-8 h-8 rounded-full bg-slate-900 border border-violet-500/50 flex items-center justify-center shrink-0 shadow-[0_0_10px_rgba(139,92,246,0.2)] mt-1 group-hover:border-fuchsia-400 transition-colors">
                <div className="w-2 h-2 rounded-full bg-violet-400 group-hover:bg-fuchsia-400 transition-colors"></div>
              </div>
              <div className="flex-1 bg-slate-900/50 border border-slate-800 rounded-xl p-4 group-hover:border-slate-600 transition-colors flex justify-between items-center gap-4">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <span className="font-mono text-sm text-fuchsia-300 bg-fuchsia-500/10 px-2 py-0.5 rounded border border-fuchsia-500/20">{cp.hash}</span>
                    <span className="text-xs text-slate-400 flex items-center gap-1">
                      <Clock className="w-3 h-3" /> {cp.timeAgo}
                    </span>
                    <span className="text-xs text-slate-500">{cp.author}</span>
                  </div>
                  <div className="text-slate-200 text-sm">{cp.message.replace(/^aicp\/[^\s]+\s*/, '')}</div>
                </div>
