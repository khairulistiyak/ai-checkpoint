...
export default function StepItem({
  ...
  return (
    <motion.div
      ...
      className={`py-1.5 px-3 rounded-xl transition-all duration-150 flex flex-col sm:flex-row sm:items-center justify-between gap-2 border group ${
        isRunning
          ? 'bg-amber-500/[0.08] border-amber-500/30 shadow-[0_0_15px_rgba(245,158,11,0.08)]'
          : isDone
    ...
    >
      <div className="flex items-center gap-2.5 min-w-0 flex-1">
        ...
        <span className={`text-[10px] font-mono shrink-0 ${isDone ? 'text-zinc-600' : 'text-zinc-400'}`}>#{step.number}</span>
        <span className={`text-xs tracking-tight truncate ${isDone ? 'text-zinc-500 line-through decoration-zinc-700' : 'text-zinc-200 font-medium'}`}>{cleanTitle}</span>
        {isRunning && (
