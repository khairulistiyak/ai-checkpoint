...
import {
  Folder, ArrowRight, AlertCircle, Cpu, Copy, Check
} from 'lucide-react';
...
export default function HomeProjectCard({ project }) {
  ...
  if (project.isInstalled && project.progress?.overall) {
        ...
        <div className="flex items-center gap-3.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <p className="text-xs font-mono text-zinc-500 truncate" title={project.path}>
                {project.path}
              </p>
              <button
              ...
              >
                {copied ? <Check className="w-3 h-3 text-workflow-success" /> : <Copy className="w-3 h-3" />}
              </button>
            </div>
          </div>
        </div>
        ...
        {project.isInstalled ? (
          <div className="space-y-2.5">
            <div className="flex items-center justify-between text-xs font-mono">
              <span className="text-zinc-400 uppercase tracking-wider font-bold">Health Index</span>
              <span className="text-white font-bold">
                {percent}% <span className="text-zinc-500">({doneSteps}/{totalSteps})</span>
              </span>
            </div>
            <div className="w-full h-1.5 rounded-full bg-white/10 overflow-hidden">
              <div
                className={`h-full transition-all duration-500 rounded-full ${percent === 100 ? 'bg-workflow-success' : 'bg-workflow-running'}`}
                style={{ width: `${percent}%` }}
              />
            </div>
            {project.unsyncedSteps > 0 && (
              <div className="flex items-center gap-1.5 pt-0.5">
                <span className="text-[10px] font-mono text-amber-400/90 font-medium">
                  ⚠ {project.unsyncedSteps} unsynced plan step{project.unsyncedSteps > 1 ? 's' : ''}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2.5 p-3 rounded-2xl bg-white/[0.02] border border-white/5">
            ...
            <span className="text-xs font-mono text-zinc-400">Not initialized. Click to initialize AI Checkpoint.</span>
          </div>
        )}
      </div>
