...
export function buildBulkIssuesPrompt(opts = {}) {
  ...
  const formatted = issues.map(iss => {
    const loc = iss.file ? `${iss.file}${iss.line ? `:${iss.line}` : ''}` : 'General';
    return `File: \`${loc}\`\nIssue: ${iss.message || iss.error || iss.msg}\nFix: ${iss.prompt || 'Apply surgical non-breaking resolution'}`;
  }).join('\n\n---\n\n');

  return [
    `# [BULK SURGICAL REMEDIATION: CATEGORY ${category.toUpperCase()}]`,
    `Please resolve the following ${issues.length} issues using strict zero-regression standards:`,
    ``,
    formatted,
    ``,
    `## Requirements:`,
    `1. Fix one issue at a time.`,
    `2. Keep all files <= 150 lines.`,
    `3. Verify every step with \`./l v && npm test\`.`
  ].join('\n');
}
